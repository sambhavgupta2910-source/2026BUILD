...
export function Badge({ children, tone = "steel" }: { children: ReactNode; tone?: "steel" | "gold" | "ok" }) {
  const color =
    tone === "gold" ? "text-gold" : tone === "ok" ? "text-ok" : "text-steel";
  return (
...
export function StatLink({ href, label, value }: { href: string; label: string; value: ReactNode }) {
  return (
    <Link href={href} className="block rounded-lg border border-line bg-card p-5 transition-colors hover:border-steel">
