...
function statusClass(s) {
  ...
  if (l.includes('limited')) return 'status-limited';
  if (l.includes('launch')) return 'status-launching';
  if (l.includes('ready') || l.includes('soon') || l.includes('final')) return 'status-ready';
...
function renderDiscover(listings) {
  ...
  const hot      = listings.filter((l) => l.status === 'Selling Fast' || l.status === 'Limited Units');

  // Hero — first featured
  const hero = featured[0];
  if (hero) {
    document.getElementById('discoverHero').style.background =
      `linear-gradient(135deg, ${hero.color || '#1a3a5c'} 0%, #050810 100%)`;
    ...
    document.getElementById('discoverHero').onclick = (e) => { if (e.target !== document.getElementById('dhCta')) openSheet(hero); };
  }

  // Hot launches horizontal scroll
  const hotEl = document.getElementById('hotLaunches');
  const hotItems = hot.length ? hot : listings.slice(0, 5);
  hotEl.innerHTML = hotItems.map((l) => `
    <div class="hot-card" data-id="${l.id}">
      <div class="hc-top" style="background:linear-gradient(135deg,${l.color||'#1a1a2e'} 0%,#050810 100%)">
  ...
  // Proof grid — DLD stats enriched on server
  ...
  const devs     = [...new Set(listings.map((l) => l.developer))].length;
  const avgOffplan = withDld.length
    ? Math.round(withDld.reduce((s, l) => s + (l.dldOffplanPct || 0), 0) / withDld.length) : 0;
  ...
  document.getElementById('proofGrid').innerHTML = `
    ...
    <div class="proof-card"><div class="proof-val">${listings.length}</div><div class="proof-label">Active Launches Listed</div></div>
    <div class="proof-card"><div class="proof-val">${areas}</div><div class="proof-label">Areas Covered</div></div>
    <div class="proof-card"><div class="proof-val">${avgOffplan}%</div><div class="proof-label">Avg Off-Plan Mix in Coverage Areas</div></div>
  `;

  // Developer chips → filter off-plan tab
  const devs2 = [...new Set(listings.map((l) => l.developer))];
  document.getElementById('devChips').innerHTML = devs2.map((d) =>
    `<button class="dev-chip" data-dev="${d}">${d}</button>`
  ).join('');
  document.getElementById('devChips').querySelectorAll('.dev-chip').forEach((btn) => {
    btn.addEventListener('click', () => {
      activeDevFilter = btn.dataset.dev;
      showScreen('offplan');
    });
  });
}
...
function renderListings() {
  filteredListings = activeDevFilter === 'All'
    ? allListings
    : allListings.filter((l) => l.developer === activeDevFilter);

  renderDevFilterChips();

  const grid = document.getElementById('listingsGrid');
  if (!filteredListings.length) {
    grid.innerHTML = '<div class="loading-msg">No listings for this developer yet.</div>';
    return;
  }

  grid.innerHTML = filteredListings.map((l) => `
    <div class="listing-card" data-id="${l.id}">
      <div class="lc-hero" style="background:linear-gradient(135deg,${l.color||'#1a1a2e'} 0%,#050810 100%)">
      ...
      <div class="lc-body">
        ...
        <div class="lc-footer">
          <div>
            ...
            ${l.dldMedianPsf ? `<div class="lc-dld">DLD: AED ${fmtN(l.dldMedianPsf)}/sqft in ${l.area}</div>` : ''}
          </div>
  ...
  grid.querySelectorAll('.listing-card').forEach((el) => {
    el.addEventListener('click', (e) => {
      if (e.target.classList.contains('lc-cta')) {
        e.stopPropagation();
        const l = allListings.find((x) => x.id === el.dataset.id);
        if (l) openSheet(l, true);
      } else {
        const l = allListings.find((x) => x.id === el.dataset.id);
        if (l) openSheet(l);
      }
...
function openSheet(listing, scrollToForm = false) {
  ...
  // Hero
  document.getElementById('sheetHero').style.background =
    `linear-gradient(135deg, ${listing.color||'#1a1a2e'} 0%, #050810 100%)`;
  document.getElementById('sheetStatus').innerHTML =
    `<span class="hc-status ${statusClass(listing.status)}">${listing.status||'Available'}</span>`;
  ...
  // Description + units + tags
  document.getElementById('sheetDesc').textContent = listing.description;
  document.getElementById('sheetUnits').innerHTML =
    listing.unitTypes.map((u) => `<span class="unit-pill">${u}</span>`).join('');
  document.getElementById('sheetTagsRow').innerHTML =
    listing.tags.map((t) => `<span class="sheet-tag">${t}</span>`).join('');

  // Unit select in lead form
  ...
  if (listing.dldMedianPsf) {
    ...
    `;
    // Fetch AI commentary async
    fetch(`/api/analysis?area=${encodeURIComponent(listing.dldAreaKey || listing.area)}`)
      .then((r) => r.json())
      .then((d) => {
        if (d.commentary) {
          ctxBody.innerHTML += `<p style="margin-top:10px;font-size:13px;line-height:1.6;color:var(--text)">${d.commentary}</p>`;
        }
      }).catch(() => {});
  } else {
    ctxBody.textContent = 'DLD transaction data for this area will appear here once available.';
  }
...
async function loadTrends() {
  ...
  try {
    ...
    document.getElementById('trendsWeek').textContent =
    ...
    document.getElementById('heatingUp').innerHTML = volumeMovers.slice(0, 6).map((m) => `
      <div class="mover-item">
        <div><div class="mover-name">${m.area}</div><div class="mover-sub">${m.weekCount} transactions · AED ${fmtN(m.medianPsf)}/sqft</div></div>
    ...
    ).join('') || '<span style="color:var(--muted);font-size:13px;padding:4px 0">No surge areas this week</span>';
  } catch { /* silent */ }
}
...
async function init() {
  try {
    ...
    document.getElementById('listingsGrid').innerHTML =
      '<div class="loading-msg">Unable to load listings. Please refresh.</div>';
  }
}
